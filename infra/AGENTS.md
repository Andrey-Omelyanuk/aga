...
- `make dev-e2e` — e2e всего рабочего цикла агента на dev-стенде
  ...
  агент-рантайма; 1-й запрос — `[ASK_HUMAN]`, дальше — финал): переключает набор
  проекта на agent-echo с mock-LLM, alice триггерит — вопрос появляется в чате
  текстом (не `Request ID`), bob (непривязанный участник) отвечает `parent_id` на
  сообщение-вопрос — запрос закрывается и агент возобновляется (канал чата сессии
  в подписке рантайма), финальный ответ от имени alice.
