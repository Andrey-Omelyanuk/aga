...
const CapabilityHistoryPage = observer(() => {
  ...
  return (
    <div>
      <Link to="/capabilities" className="text-xs text-blue-600 hover:underline">
        ← Каталог
      </Link>
      <Card>
        ...
        {error ? (
          ...
          <div className="space-y-1.5">
            {history.map((entry) => (
              <div
                key={entry.id}
                className="flex items-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs text-slate-600"
              >
