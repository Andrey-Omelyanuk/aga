...
- **Делает:** вкладки Проекты / Воркстейшны / Сессии / Файлы / Чат / Config
  ...
  статическая справка о составе набора агентов (диаграмма дерева) и о составе
  всего конфига, на русском и английском с переключателем; отправка сообщений, дерево
  сообщений, артефакты,
...
- **Справка Help — статическая документация, не данные:** `ConfigHelpPage`
  (`/config/help`, `components/settings/HelpContent` + `AgentSetDiagram`) не ходит
  к API и не зависит от настроенного — текст и диаграмма одинаковы при любой
  настройке платформы. Диаграмма состава набора — HTML-дерево без библиотек
  диаграмм. Язык справки (ru/en, по умолчанию ru) хранится в localStorage
  (`aga_help_lang`), поэтому переживает переходы по страницам.
...
- Unit (vitest): модели/фильтры (`workstation.test.ts`), утилиты
  ...
  набора, описание всего конфига, русский при первом открытии, переключатель
  en/ru с сохранением выбора, страница не ждёт данных). Запуск: `npm test`.
