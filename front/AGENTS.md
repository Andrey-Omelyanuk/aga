...
- **Делает:** вкладки Проекты / Воркстейшны / Сессии / Файлы / Чат / Config
  ...
  английском с переключателем; отправка сообщений, дерево
  сообщений, артефакты,
...
- **Чат:** `useQuery(Chat, {autoupdate:true})` для списка; текущий чат —
  ...
  participants}` — разворачивается в плоский объект модели). Обновления — по
  websocket: `pub_sub.on_message` на общем канале `common` перезагружает деталь
  открытого чата и список; polling'а нет (см. историю chat-websocket-centrifuge).
...
- Unit (vitest): модели/фильтры (`workstation.test.ts`), утилиты
  ...
  общего канала, сообщение в другом чате открытый не трогает, polling'а по
  таймеру в странице нет, у сессии есть ссылка на Changes, у чата без
