...
- **Делает:** вкладки Проекты / Наборы / Воркстейшны / Сессии / Персонал /
  Файлы / Чат / Настройки; создание проектов, открытие/закрытие сессий на
  воркстейшнах,
  ...
  и редактор состава — агенты, их территория, данные скиллы и команды по имени
  (без версии), инструменты; страница «Каталог»: скиллы и команды с единственным
  текущим содержимым (правка перезаписывает его и пишется в историю), список
  «Удалённые» (удалённые записи переживают в истории); страница истории на
  конкретную запись (`/skills/:id/history`, `/commands/:id/history`) — кто, когда
  и что сделал, по порядку; отправка сообщений, дерево сообщений, артефакты,
  опрос для ответов реактивных агентов; вход через SSO; страница «Настройки» —
  публичный SSH-ключ aga (копирование; ключ задаёт админ в env ядра).
...
- Unit (vitest): модели/фильтры (`workstation.test.ts`), утилиты
  ...
  историю, список «Удалённые» с открываемой историей). Запуск: `npm test`.
