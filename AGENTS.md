...
SPA на nginx). Стенд — в Kubernetes (`infra/`). Спроектирован для слабых
...
- `[ASK_HUMAN]` — единственный протокол human-in-the-loop, живёт в чате: текст вопроса
...
- Чат ничего не знает про агентов: HTTP-сервер пишет сообщения и публикует
  ...
  отдельный процесс того же бинаря (`aga agent`), подписанный на каналы
  пользователей; привязка `agents.listen_user_id` (кого слушать и от чьего
  имени отвечать) настраивается на странице набора. `@Agent.<имя>` триггером
  не является.
