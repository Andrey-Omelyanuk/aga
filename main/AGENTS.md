...
└── server.rs     # Axum Router + хендлеры API + SSE для human-запросов + /auth/login, /auth/callback, /auth/refresh, /auth/logout, /users/me
```
...
- **Жизненный цикл воркстейшна:** при подъёме в ws может монтироваться именованный
  ...
  clone идёт во временный каталог, подмена — только при успехе). В dev (docker) перед
  клоном ядро инжектит SSH-ключ aga в контейнер (`inject_ssh_key`) — станция
  переиспользуется, и ключ из `create_workstation` в неё не попадал. Отпустить станцию
...
- **LLM — выбранное подключение, не модель агента:** подключения к LLM
  (`llm_connections`: имя, url API, ключ доступа) живут в БД и настраиваются
  через API (`/llms`). Агент набора ссылается на подключение (`agents.llm_id`);
  своей модели и температуры у агента больше нет (старые столбцы
  model/temperature остаются в старых БД, но не используются) — модель
  дефолтная из `LLM_MODEL`, температура 0.7. url и ключ агента берутся из
  подключения в момент запуска (`TraceStore::llm_config_for`); без подключения
  (или при ссылке на удалённое) — дефолтные url/ключ из env. Правка
  подключения меняет url/ключ сразу у всех агентов, что на него ссылаются;
  удаление не ломает агентов — они остаются на env-дефолте (на новых БД каскад
  `ON DELETE SET NULL`, на старых ссылка просто не находится). Ключ наружу
  отдаётся как есть, маскировки нет.
